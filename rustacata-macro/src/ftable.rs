...
fn gen_enum(args: &Args, item: &ItemEnum) -> TokenStream {
    let name = ftable_name(&item.ident);
...
fn gen_variant_setter(item: &ItemEnum, variant: &Variant) -> TokenStream {
    let alg_name = ftable_name(&item.ident);
    let field_name = variant_field_name(&variant.ident);
...
fn ftable_name(ident: &Ident) -> Ident{
    Ident::new(&format!("{}FTable", *ident), Span::call_site())
}

fn variant_field_name(ident: &Ident) -> Ident {
    Ident::new(&format!("f_{}", *ident).to_lowercase(), Span::call_site())
}

fn variant_setter_name(ident: &Ident) -> Ident {
    Ident::new(&format!("with_f_{}", *ident).to_lowercase(), Span::call_site())
}
